...
def readTableRules(p4info_helper, sw):
    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            name = p4info_helper.get_name("tables", entry.table_id)
            #print(name)
            print(entry)
            print('-----')
def readDigest(p4info_helper, sw):
    """
    Reads the table entries from all tables on the switch.
    ...
    while True:
        ...
        for digest in sw.ReadDigest(digest_id, list_id):
            ...
            for data in digest.data:
                ...
                interval = int.from_bytes(data.struct.members[2].bitstring, 'little') / packet_count
